// title: ballast — granular-ish synth (no samples, JI base)
export async function start({ Tone, params, out }) {
  ...
  const shaper = new Tone.Saturator(0.05);
  const osc = new Tone.Oscillator({ frequency: baseHz, type:'sawtooth' }).start();

  const vca = new Tone.Multiply(0); const env = new Tone.TimelineSignal(0);
  osc.connect(shaper); shaper.connect(hp); hp.connect(vca); vca.connect(lp); lp.connect(vol); vol.connect(out);
  ...
  const gClock = new Tone.Clock(()=>{
    ...
    env.setValueAtTime(0, now+ofs);
    env.linearRampToValueAtTime(1, now+ofs+glen*0.25);
    env.exponentialRampToValueAtTime(0.0001, now+ofs+glen);
  }, grains);
  gClock.start();
  ...
  return ()=>{ on=false; clearTimeout(t); try{ gClock.stop(); gClock.dispose(); [osc, shaper, vca, env, hp, lp, vol].forEach(n=>n.dispose?.()); }catch(_){} };
}
